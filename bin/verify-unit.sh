...
# tree (the role's worktree), tees output to .team/verify/<unit>.log, and exits
# with the command's status. A role's `done:` is only valid with a fresh green
# log from this script; this is "done means verified" as an exit code, not prose.
#
...
mkdir -p "$repo/.team/verify"
log="$repo/.team/verify/$unit.log"
{
